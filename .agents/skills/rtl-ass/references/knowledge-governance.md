...
Retrieval is the primary RTL-ASS coding-time mechanism. When an in-scope database exists, inspect its audited namespaces and search the project namespace first, then explicitly applicable user, organization, or built-in namespaces. Search `promoted` records first and fall back to `verified` only when no relevant promoted result exists. Never retrieve `raw` or `candidate` records as default coding guidance. Limit each receipt to three results and open at most three full records across the task. Use `--match all` for a precise short query and explicit `--match any` for exploratory multi-term retrieval; there is no hidden fallback between them. Prefer matching role, language, interface/protocol, clock/reset assumptions, and verification state over textual similarity alone. Inspect source, license, limitations, and negative evidence before reuse.
