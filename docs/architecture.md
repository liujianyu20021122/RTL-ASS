...
The MVP uses SQLite FTS5 plus structured filters. Vector retrieval is deferred until it demonstrates measurable value over lexical and RTL-structural signals. Codex receives bounded result cards and decides whether to inspect or reuse them. An immutable retrieval receipt binds the actor, query, explicit namespaces, filters, limit, ordered result identities, content hashes, provenance, license state, and ranking data; opening full record content remains a separate observable action.
...
The workflow boundary is separate from backend execution. A verification plan records claims selected by Codex, their required/optional status, expected evidence class, and bounded retry/concurrency policy. The summary operation accepts explicit claim-to-evidence links, reuses central run/waveform validators, rechecks current files, and computes readiness without choosing an RTL patch. High-cost CLI evidence operations share one bounded advisory lock under the current workspace so separate Codex commands cannot run EDA backends concurrently by default.
